...
def refactor_codebase(path: Path, goal: str, language: str | None, console: Console) -> RefactorReport:
    ...
    if not LANGUAGE_REGISTRY.get(language):
        console.print(
    ...
    all_files = _collect_files(path, language)
    ...
    console.print("[dim]Generating refactor plan...[/dim]")
    ...
    try:
        ...
        changes = [RefactorChange(path=change["path"], content=change["content"]) 
        ...
        raise RuntimeError("Failed to parse refactor response") from e
